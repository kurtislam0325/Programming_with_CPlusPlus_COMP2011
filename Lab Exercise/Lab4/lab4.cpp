    ...
    */
    ...
    for (int j = 0; j < MAX_QUEUE_LENGTH; j++)
    {
        if (queue[j] == 0)
        {
            ...
            next_ticket_number = queue[j] + 1;
            return true;
        }
        if (j == MAX_QUEUE_LENGTH - 1 && queue[j] != 0)
        {   
            return false;       
        }           
    }
...
bool serve_the_first_come_customer(int queue[]){
    ...
    */
    ...
    int queue_number_sum = 0;

    for (int j = 0; j < MAX_QUEUE_LENGTH; j++)
        queue_number_sum += queue[j];

    if (queue_number_sum == 0)
        return false;
    else 
    {
        queue[0] = 0; queue[MAX_QUEUE_LENGTH] = 0;
        for (int j = 0; j < MAX_QUEUE_LENGTH; j++)  
            queue[j] = queue[j+1];
        return true;
    }
